...
//   {
//     title: "Nairobi Mart",
//     description: "A market place where customers can buy from different vendors",
//     image: "/NB.png",
//     link: "https://nairobimart.vercel.app/"
//   },
//   {
...
export default function Projects() {
  ...
  useEffect(() => {
    ...
    const unsubWeb = onSnapshot(
      query(collection(db, "webProjects"), orderBy("createdAt", "desc")),
      snap => { setWebProjects(snap.docs.map(d => ({ id: d.id, ...d.data() }))); done(); }
    );
    const unsubSeo = onSnapshot(
      query(collection(db, "seoProjects"), orderBy("createdAt", "desc")),
      snap => { setSeoProjects(snap.docs.map(d => ({ id: d.id, ...d.data() }))); done(); }
    );
    const unsubDesign = onSnapshot(
      query(collection(db, "designProjects"), orderBy("createdAt", "desc")),
      snap => { setDesignProjects(snap.docs.map(d => ({ id: d.id, ...d.data() }))); done(); }
    );

    return () => { unsubWeb(); unsubSeo(); unsubDesign(); };
  }, []);

  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-r from-black">
        <div className="w-10 h-10 border-4 border-orange-100/20 border-t-red-700 rounded-full animate-spin" />
      </div>
    );
  }
    ...
    >
      ...
      {webProjects.length > 0 && (
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 mb-12">
            {webProjects.map((proj, i) => (
              <ProjectCard proj={proj} i={i} key={proj.id} seoProjects={seoProjects} />
            ))}
          </div>
        </>
      )}
      ...
      {!loading && webProjects.length === 0 && seoProjects.length === 0 && designProjects.length === 0 && (
        <p className="text-center text-orange-100/40 py-24">No projects yet.</p>
      )}
    </motion.div>
  );
}
...
function ProjectCard({ proj, i, seoProjects }) {
  const Stat = ({ label, value, change, extra }) => (
    <div>
      <p className="text-gray-400">{label}</p>
      <p className="font-bold text-lg">
      ...
      <div className="flex flex-col flex-1 px-6 py-5">
        ...
        {proj.stats && (
          <div className="grid grid-cols-2 gap-4 text-white text-sm mb-4">
            <Stat
              ...
              extra={`💻 ${proj.stats.visits.devices.desktop} / 📱 ${proj.stats.visits.devices.mobile}`}
            />
            <Stat label="Unique Visitors" value={proj.stats.uniqueVisitors.value} change={proj.stats.uniqueVisitors.change} />
            <Stat label="Conversion" value={proj.stats.conversion.value} change={proj.stats.conversion.change} />
            <Stat label="Pages / Visit" value={proj.stats.pagesPerVisit.value} change={proj.stats.pagesPerVisit.change} />
            <Stat label="Avg. Duration" value={proj.stats.avgVisitDuration.value} change={proj.stats.avgVisitDuration.change} />
            <Stat label="Bounce Rate" value={proj.stats.bounceRate.value} change={proj.stats.bounceRate.change} />
          </div>
        )}
        ...
        {proj.link && (
          <>
            ...
            {isSeoProject && (
              <div className="mt-3 flex items-center justify-center space-x-2">
                <img src="/sem.jpg" alt="Semrush Logo" className="h-5" />
                <span className="text-xs text-gray-400">Powered by Semrush</span>
              </div>
            )}
          </>
